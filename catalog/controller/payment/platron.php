...
class ControllerPaymentplatron extends Controller {
	protected function index() {
		...
		$this->data['action'] = 'https://www.platron.com/PayProcess.aspx';

		$this->data['ap_merchant'] = $this->config->get('platron_merchant');
	...
	public function callback() {
		if (isset($this->request->post['ap_securitycode']) && ($this->request->post['ap_securitycode'] == $this->config->get('platron_security'))) {
			$this->load->model('checkout/order');
			
			$this->model_checkout_order->confirm($this->request->post['ap_itemcode'], $this->config->get('platron_order_status_id'));
		}
	}
}
?>